...
def prepare_ly(ly_path, kodourc, paperpart_global_ly_commands):
    ...
    with open(kodourc, "r") as f:
        for line in f.read().splitlines():
            if line and not line.startswith("#"):  # pound sign is .kodou comment
                k, v = line.split("=")
                dot_kodou_commands[k.strip(" ")] = v.strip(" ")  # strip whitespaces
    with open(ly_path, "w") as f:
        ...
        for command in paperpart_global_ly_commands:
            ...
            f.write("\n")
        f.write("\n" * 4)
    viewer = dot_kodou_commands.get("viewer", "/usr/bin/evince")
    lilypond = dot_kodou_commands.get("ly_exe", "/usr/local/bin/lilypond")
